		...
		else if (param == "mesh")
		{
			...
			while (param != "}")  // Read until we get to the '}' character
			{
				if (param == "shader")
				{
					...
					std::string path_to_texture = "Models//Human//" + mesh.m_Shader;
					//path_to_texture = "Models//Human//brick_texture.tga";
					//std::cout << mesh.m_Shader << std::endl;
					mesh.m_TexID = SOIL_load_OGL_texture(path_to_texture.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS);
...
void MD5Model::Render()
{
	...
	m_Animation.Render();
	//renderowanie normali, odkomentowac te 3 linijki zeby je widizec
	/*for ( unsigned int i = 0; i < m_Meshes.size(); ++i )
	{
